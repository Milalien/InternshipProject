...
if(isset($_GET["la"]) && $_GET["la"]!= $_SESSION["la"]){
	...
	$lang = $_SESSION["la"];
        ...
        $mokki2 = $result->fetch_array(MYSQLI_ASSOC);
        $stmt->close();
		...
		echo "<div class='subPage2'>
            ...
            echo "<div class='tabs'>";
            echo "<div class='tab'>
                          <img class='demo cursor' src='images/kailantoThumb.jpg' style='width:100%' onclick='currentSlide(0)' alt=''>
                  </div>";
            $stmt = $conn->prepare("SELECT id, jarjestys, kuva FROM mokki2_slider_kuvat ORDER BY jarjestys;");
                    $stmt->execute();
                    $result = $stmt->get_result();
            ...
            echo "</div>
			<div class='text-area'>
				".$mokki2["esittelyteksti"]."";
				
				if($mokki2["varausOsoite"] != null){
				    echo "<a class='btn' href=".$mokki2["varausOsoite"]." role='button'>".$mokki2["nappi1"]."</a>";
				}
                    
                    if($mokki2["nappi2"] != ""){
                        echo "<button type='button' class='btn' id='priceBtn1'>".$mokki2["nappi2"]."</button>";
                    }
                    ...
                    <div class='btm'>
                        <div class='location'>
                            <h3>".$mokki2["alaOtsikko1"]."</h3>
                            ".$mokki2["sijainti"]."
                            ".$mokki2["kartta"]."
                        </div>
                        <div>
                            <h3>".$mokki2["alaOtsikko2"]."</h3>
                           ".$yhteystiedot."
                        </div>
                    </div>
                </div>
